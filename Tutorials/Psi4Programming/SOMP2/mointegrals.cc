 ...
 * @BEGIN LICENSE
 *
 * mp2so_plug by Psi4 Developer, a plugin to:
 *
...
extern "C" int read_options(std::string name, Options& options) {
    if (name == "MP2SO_PLUG" || options.read_globals()) {
...
extern "C" SharedWavefunction mp2so_plug(SharedWavefunction ref_wfn, Options& options) {
    /*
